(ns rtgs_global.main
  "Kotodama WASM entrypoint for the RTGS Global clean-room actor (L4) — Clojure port.
  ...
  facts (`rtgs_global.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rtgs_global")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rtgs_global.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rtgs_global-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rtgs_global-compat/ready)
